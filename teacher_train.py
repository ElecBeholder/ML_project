...
from teacher.pytorch_shake_shake.dataloader import get_loader
...
def main():
    ...
    for epoch in range(1, optim_config['epochs'] + 1):
        ...
        # 保存最好的模型
        current_accuracy = test_log['test']['accuracy']
        if current_accuracy > best_accuracy:
            best_accuracy = current_accuracy
            best_model_path = outdir / 'best_model_state.pth'
            torch.save(state, best_model_path)
            logger.info(f'Saved best model, accuracy: {best_accuracy:.4f}, epoch: {epoch}')
